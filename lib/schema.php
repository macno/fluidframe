...
class Schema
{
    ...
    function appendDropFkIndex (array &$statements, $table, $name)
    {
        $statements[] = 'ALTER TABLE ' . $table . ' DROP FOREIGN KEY ' . $name ;
    }
    ...
    function buildEnsureTable($tableName, array $def)
    {
        ...
        foreach ($foreign['del'] + $foreign['mod'] as $keyName) {
            $this->appendDropFkIndex($statements, $tableName, $keyName);
        }
