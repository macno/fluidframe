...
class MysqlSchema extends Schema
{
    ...
    function getCharset() {
	if(!$this->charset) {
		$this->charset = (common_config('db','mysql_charset')) ? common_config('db','mysql_charset') : $this->DEFAULT_CHARSET;
	}
	return $this->charset;
    }
    /**
    ...
    public function getTableDef($table)
    {
        ...
        if (common_config('db', 'mysql_foreign_keys')) {
            $fkInfo = $this->fetchFkInfo($table);
            if($fkInfo) {
                $def['foreign keys'] = $fkInfo;
            }
        }
        return $def;
    }
     ...
     * @return array of arrays
     */
    function fetchMetaInfoConstraint($constraint, $infoTable, $orderBy=null)
    {
        $query = "SELECT * FROM INFORMATION_SCHEMA.%s " .
    ...
    function fetchFkInfo($table) {
        $ret = false;
        $contraints = $this->fetchMetaInfo($table, 'TABLE_CONSTRAINTS');
        $fks = array();
        foreach ($contraints as $contraint) {
            if($contraint['CONSTRAINT_TYPE'] == 'FOREIGN KEY') {
                $fks[] = $this->fetchMetaInfoConstraint($contraint['CONSTRAINT_NAME'], 'REFERENTIAL_CONSTRAINTS');
            }
        }
        ...
        if(count($fks)>0) {
            ...
            foreach ($fks as $id => $fkz) {
                ...
                $colDefz = $this->fetchMetaInfoConstraint($fk['CONSTRAINT_NAME'], 'KEY_COLUMN_USAGE');
                $colDef = $colDefz[0];
                
                $options = '';
                if($fk['DELETE_RULE'] != 'RESTRICT') {
                    $options .= 'ON DELETE ' . $fk['DELETE_RULE'];
                }
                if($fk['UPDATE_RULE'] != 'RESTRICT') {
                    $options .= 'ON UPDATE ' . $fk['DELETE_RULE'];
                }
                $ret[$fk['CONSTRAINT_NAME']] = array(
                        $fk['REFERENCED_TABLE_NAME'],
                        array(
                                $colDef['COLUMN_NAME'] => $colDef['REFERENCED_COLUMN_NAME']
                    )
                );
                if(!empty($options)) {
                    $ret[$fk['CONSTRAINT_NAME']][] = $options;
                }
            }
        }
        return $ret;
    ...
    function endCreateTable($name, array $def)
    {
        ...
        return ") ENGINE=$engine CHARACTER SET ". $this->getCharset() ." COLLATE ". $this->getCharset() ."_bin";
    }
    ...
    function appendAlterExtras(array &$phrase, $tableName, array $def)
    {
        ...
        if (strtolower($oldProps['TABLE_COLLATION']) != $this->getCharset() . '_bin') {
            $phrase[] = 'CONVERT TO CHARACTER SET '. $this->getCharset() .' COLLATE '. $this->getCharset() .'_bin';
            $phrase[] = 'DEFAULT CHARACTER SET = ' . $this->getCharset();
            $phrase[] = 'DEFAULT COLLATE = '. $this->getCharset() .'_bin';
        }
    }
